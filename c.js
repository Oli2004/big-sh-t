...
console.log("Welcome, Fellow Admin");
console.log("Updates/News:BOT IDS !!!!");
console.log("------------------------------------");
console.log("Now scanning DATA FILES");
console.log("Finding local Files");
console.log("Files has been found Proceeding");
console.log("Welcome To Project NoName");
console.log("Works on over 10+ Clones!");
console.log("Found service = Codeanywhere");
...
function FeederBot(bot_id, agent, bot_number, server, origin) {
    ...
    this.client = new AgarioClient('Prox|'+ this.bot_id + '|Feeder'); 
    this.client.debug = 0;
...
FeederBot.prototype = {
    log: function(text) {
        if (config.verbosityLevel > 0) {
            console.log(' Pyro' + this.bot_id + ': ' + text);
        }
    },
    ...
    attachEvents: function() {
        ...
        bot.client.on('disconnect', function() {
            ...
            if (spawnCount > 200){ spawnCount--; bot_count--;}
...
socket.on('cmd', function(data) {
    if (data.name == "split") {
		...
		origin = data.origin;
        console.log("NoName Bots spawning bots on " + game_server_ip);
        ...
        game_server_ip = data.ip;
		origin = data.origin;
        console.log("NoName Bots starting bots  on " + game_server_ip);
	}
});
...
function startFeederBotOnProxies() {
    ...
    for (proxy_line in lines) {
        ...
        if(lines[proxy_line].trim() == "#HTTP"){
            proxy_mode = "HTTP";
				}else if(lines[proxy_line].trim() == "#HTTP"){
            proxy_mode = "HTTP";
				}else if(lines[proxy_line].trim() == "#HTTPS"){
            proxy_mode = "HTTPS";
